...
import test4Data from '@/data/test4Data'
...
export default function TestCuatroForm() {
      ...
      <React.Fragment>
        <div className="flex flex-col mt-2 items-start justify-start md:mt-8 md:flex-row md:items-center md:justify-center md:space-x-8 text-gray-900 dark:text-gray-300">
          <form method="post" onSubmit={handleSubmit}>
            <div className="tracking-tight text-4xl font-bold text-gray-800 dark:text-gray-100">En mantenimiento
              <div className="tracking-tight px-1 text-base font-bold text-gray-600 dark:text-gray-400"> Versión 4</div></div>
            {/* PREGUNTA FORMULARIO */}
            <br></br>
            <div className="flex flex-wrap">
              <Box sx={{ minWidth: 220, maxWidth: 600 }}>

                <List component="div" variant="plain" size="sm" sx={{ borderRadius: 'none', boxShadow: 'base' }}>

                  {test4Data.map((jsonPreg, index) => (
                    <div className=" border-gray-900/10 dark:border-gray-100/20 text-black dark:text-gray-100 pb-2" key={jsonPreg}>

                      {/* Pregunta form */}
                      <div className="pt-2 tracking-wides items-center text-base font-sans font-bold text-sky-600">Pregunta {index + 1} </div>
                      <div className="tracking-wides items-center pb-2 text-base font-sans font-normal text-black dark:text-white">
                        {jsonPreg.PREG}
                      </div>

                      {/* <div className="tracking-wides grid grid-cols-2 py-2 text-base font-sans font-normal text-black dark:text-white"> */}
                      <div className="select-none grid grid-cols-2" >
                        <ImageComponent
                          iClassName="justify-items-start"
                          iSrc={jsonPreg.IMGT}
                          iWidth={200}
                          iHeight={200}
                          iAlt="Alumnos"
                        />
                        {/* </div> */}
                        {/* <div className="tracking-wides columns-1 space-x-2 py-1 leading-5 text-base font-sans font-normal text-black dark:text-white"> */}
                        {/* Opciones form */}
                        <RadioGroup aria-labelledby="example-payment-channel-label" style={{
                          minWidth: '200px', minHeight: '190px', display: 'flex',

                        }} overlay name={`preg${index + 1}`}>
                          {[jsonPreg.OP1, jsonPreg.OP2, jsonPreg.OP3, jsonPreg.OP4, jsonPreg.OP5].map((value, index) => (
                            value && (
                              <React.Fragment key={value}>
                                ...
                                <ListItem>
                                  <label>
                                    <Radio className="cursor-pointer flex leading-4 text-base justify-items-start font-normal text-black dark:text-gray-100 dark:hover:text-primary-500"
                                      id={value}
                                      value={value}
                                      label={value} />
                                  </label>
            ...
            <div>
              <div className="flex-col h-26 items-center py-6">
                <div className="d-grid gap-2">
                  ...
                  <div id="grupo1">
                    <Button className="w-full rounded-md dark:bg-blue-600 px-12 py-2 text-base font-semibold leading-6 text-white" type="submit"
        ...
        <br></br>
        {/* <Link href="/test-online" className="font-semibold dark:hover:text-indigo-500 text-primary-700">
          &larr; Atrás
        </Link> */}

        <ModeToggle />
        <Modal open={open} onClose={() => setOpen(false)} className="tracking-wides py-1 leading-5 text-base font-sans dark:bg-[#0d1117]">
          <Sheet sx={{ maxWidth: '99%', maxHeight: '96%', overflowY: 'scroll', mx: 1, my: 2, py: 4, px: 1, display: 'flex', flexDirection: 'column', gap: 1, borderRadius: 'sm', boxShadow: 'md' }}
            className="tracking-wides py-1 leading-5 text-base font-sans dark:bg-[#0d1117]" variant="outlined">
            <div className="text-3xl font-bold text-gray-100 py-2">
              <span className="box-decoration-clone from-indigo-900 to-slate-800 text-white">
                Corrección
              </span>
            </div>
...
function datosPrueba(frm) {
  ...
  for (let i = 0; i < ops.length; i++) {
    o.push(test4Data[i][ops[i]]);
    p.push(test4Data[i].PREG);
  }
