      ...
      <div className="bg-[#111928] p-4 rounded-lg max-w-sm w-full mx-4">
        ...
        <div className="space-y-2">
          {question.options.map((option, index) => (
            <button key={index} className="w-full text-sm tracking-tight leading-tight text-left p-2 bg-white hover:bg-btndi hover:text-white text-[#0d1117] font-medium rounded transition-colors" onClick={() => onAnswer(index)}>
              {option}
            </button>
          ))}
        </div>
