...
const Navbar = () => {
  ...
  return (
    <div className="container fixed min-w-screen top-0 z-120 border-b border-violet-200/10  backdrop-blur dark:bg-(--color-section-3)/90 dark:border-gray-600/20
     text-gray-100 ">
        ...
        <nav className="hidden md:flex gap-6 items-center text-gray-100">
          {navLinks.map((link) => (
            <Link key={link.title}
              ...
              className="relative font-medium transition-colors duration-500 after:absolute justify-end right-0 after:right-0 after:bottom-0 after:h-0.5 after:w-0 after:bg-(--color-purple-1) after:transition-all hover:after:w-full hover:text-(--color-purple-1)">
              {link.title}
            </Link>
          ))}
            ...
            <div className="relative">
              ...
              {userMenuOpen && (
                <div className="absolute mt-1 w-33 py-1 bg-white dark:bg-slate-800 rounded-md shadow-lg border border-slate-200 dark:border-slate-700 z-50">
                  ...
                  <button onClick={handleLogout} className="flex items-center w-full text-left px-4 py-2 text-sm text-violet-400 hover:bg-slate-100 dark:hover:bg-slate-700">
