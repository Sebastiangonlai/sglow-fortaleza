  ...
  return (
    <section className="h-screen overflow-hidden flex items-center justify-center">
      <div className="min-w-screen h-screen animated fadeIn faster fixed left-0 top-0 flex justify-center items-center inset-0 z-50 outline-none focus:outline-none bg-no-repeat bg-center bg-cover"
        style={{ backgroundImage: "/static/images/hero-bg.jpg" }}
        id="modal-id"      >
              ...
              <div className="flex flex-col">
                ...
                <div className="flex-auto justify-evenly">
                  ...
                  <div className="lg:flex py-4 text-sm text-gray-600">
                    <div className="flex-1 inline-flex items-center mb-3">
                      <ul className="flex flex-row justify-center items-center space-x-2">
                        {["blue-600", "yellow-400", "red-500", "green-500"].map((color, i) => (
                          <li key={i}>
                            <span className={`block p-1 border-2 border-gray-900 hover:border-${color} rounded-full transition ease-in duration-300`}>
                              <a href={`#${color}`} className={`block w-3 h-3 bg-${color} rounded-full`}></a>
                            </span>
                          </li>
                        ))}
                      </ul>
                    ...
                    <div className="flex-1 inline-flex items-center mb-3">
                      ...
                      <div className="cursor-pointer text-gray-400">
                        {["S", "M", "L", "XL"].map((size) => (
                          <span key={size} className="hover:text-purple-500 p-1 py-0">{size}</span>
                        ))}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex space-x-2 text-sm font-medium justify-start">
                    <button className="transition ease-in duration-300 inline-flex items-center text-sm font-medium bg-purple-500 px-5 py-2 hover:shadow-lg tracking-wider text-white rounded-full hover:bg-purple-600">
                      Add Cart
                    </button>
